...
# Exercise 5: Using Pandacs for data analysis and visualization

# I found two ways to get a query into a Pandas dataframe:

# 1. Make the SQL connection with SQLAlchemy's sessionmaker and put the result into a dataframe (Mikael's way)
df1 = pd.DataFrame(sql_runner.run_query(query))

# 2. Use Panda's read_sql_query directly on the connection_string (Emil's way)
...
# Barplot of total sales and revenue per country
...
sns.barplot(data=df, y=df.country, x=df.total_sales, orient='h', ax=ax[0], hue=df.total_sales, palette=palette, legend=False)
sns.barplot(data=df, y=df.country, x=df.revenue, orient='h', ax=ax[1], hue=df.revenue, palette=palette, legend=False)
...
ax[1].set_xticks([0, 50000, 100000, 150000, 200000, 250000], ['0', '50K', '100K', '150K', '200K', '250K'], font=other_font_light, fontsize=12)
...
sns.scatterplot(data=df, x=df.units_sold, y=df.avg_unit_price, hue=df.revenue, palette=palette, ax=ax, size=df.revenue, sizes=(10, 400), hue_norm=(0, 150000), size_norm=(0, 150000), legend=False)
...
#palette = load_cmap("magma_r", cmap_type="continuous")
#palette = load_cmap("lumina", cmap_type="continuous")
#palette = load_cmap("sPBIYlGn", cmap_type="continuous")
